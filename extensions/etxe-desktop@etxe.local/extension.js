...
export default class DING extends Extension {
    constructor(metadata) {
        ...
        */
        ...
        this.data.nativeDesktopMenu = null;
        this.data.nativeDesktopMenuManager = null;
        this.data.nativeDesktopMenuSource = null;
        this.data.nativeDesktopMenuSignalId = 0;
    ...
    disable() {
        ...
        if (this.data.showDesktopMenuId) {
            this.data.showDesktopMenu.disconnect(this.data.showDesktopMenuId);
            this.data.showDesktopMenuId = 0;
            this.data.showDesktopMenu = undefined;
        }
    ...
    innerEnable() {
        ...
        */
        this.data.dbusConnectionId = Gio.bus_own_name(Gio.BusType.SESSION, 'local.etxe.DesktopExtension', Gio.BusNameOwnerFlags.NONE, null, (connection, name) => {
            ...
            this.data.showDesktopMenu = new Gio.SimpleAction({
            ...
            this.data.showDesktopMenuId = this.data.showDesktopMenu.connect('activate', (action, parameters) => this.showNativeDesktopMenu(parameters));
            ...
            this.data.actionGroup.add_action(this.data.showDesktopMenu);
            ...
            this.data.dbusConnectionGroupId = this.data.dbusConnection.export_action_group(
                '/local/etxe/DesktopExtension/control',
    ...
    showNativeDesktopMenu(parameters) {
        const state = parameters.recursiveUnpack();
        const x = Number(state.x ?? 0);
        const y = Number(state.y ?? 0);

        this.destroyNativeDesktopMenu();

        const source = new St.Widget({reactive: false});
        source.set_size(1, 1);
        source.opacity = 0;
        source.set_position(
            Math.max(0, Math.min(global.stage.get_width() - 1, Math.round(x))),
            Math.max(0, Math.min(global.stage.get_height() - 1, Math.round(y)))
        );
        ...
        this.data.nativeDesktopMenu = menu;
        this.data.nativeDesktopMenuManager = manager;
        this.data.nativeDesktopMenuSource = source;
        this.data.nativeDesktopMenuSignalId = menu.connect('open-state-changed', (popup, isOpen) => {
            if (!isOpen) {
                this.destroyNativeDesktopMenu();
            }
        });
        ...
        for (const item of state.items ?? []) {
            ...
            this.addNativeDesktopMenuItem(menu, item);
        }
    ...
    activateDesktopAppAction(actionName) {
        const appActions = Gio.DBusActionGroup.get(
            Gio.DBus.session,
            ETXE_DESKTOP_APP_ID,
            ETXE_DESKTOP_APP_PATH
        );
        appActions.activate_action(actionName, null);
    }

    destroyNativeDesktopMenu() {
        if (this.data.nativeDesktopMenu) {
            if (this.data.nativeDesktopMenuSignalId) {
                this.data.nativeDesktopMenu.disconnect(this.data.nativeDesktopMenuSignalId);
                this.data.nativeDesktopMenuSignalId = 0;
            }
            this.data.nativeDesktopMenu.destroy();
            this.data.nativeDesktopMenu = null;
        }
        this.data.nativeDesktopMenuManager = null;
        if (this.data.nativeDesktopMenuSource) {
            this.data.nativeDesktopMenuSource.destroy();
            this.data.nativeDesktopMenuSource = null;
        }
    }
    ...
    */
     ...
     */
    launchDesktop() {
        console.log('Launching Etxe Desktop process');
        let argv = [];
...
class LaunchSubprocess {
    ...
    spawnv(argv) {
        try {
            ...
            this.subprocess = null;
            console.log(`Error while trying to launch Etxe Desktop process: ${e.message}\n${e.stack}`);
        }
