...
var DesktopManager = class {
    constructor(mainApp, dbusManager, desktopList, codePath, asDesktop, primaryIndex) {
        ...
        try {
            ...
            this._errorWindow = new ShowErrorPopup.ShowErrorPopup(_('Nautilus File Manager not found'),
                _('The Nautilus File Manager is mandatory to work with Etxe Desktop.'),
                true);
        }
        ...
        this._addMainAppAction('changeDesktopIconSettings', () => Prefs.showPreferences());
        this._addNativeDesktopMenuActions();
    }

    _addMainAppAction(name, callback) {
        const action = Gio.SimpleAction.new(name, null);
        action.connect('activate', callback);
        this.mainApp.add_action(action);
    }

    _addNativeDesktopMenuActions() {
        this._addMainAppAction('desktop-new-folder', () => this.doNewFolder());
        this._addMainAppAction('desktop-new-text-file', () => this._doNewTextFile());
        this._addMainAppAction('desktop-paste', () => this._doPaste());
        this._addMainAppAction('desktop-undo', () => this._doUndo());
        this._addMainAppAction('desktop-redo', () => this._doRedo());
        this._addMainAppAction('desktop-select-all', () => this._selectAll());
        this._addMainAppAction('desktop-arrange-icons', () => this._sortAllFilesFromGridsByPosition());
        this._addMainAppAction('desktop-show-in-files', () => this._onOpenDesktopInFilesClicked());
        this._addMainAppAction('desktop-open-terminal', () => this._onOpenTerminalClicked());
        this._addMainAppAction('desktop-change-background', () => this._launchControlCenterPanel('gnome-background-panel.desktop'));
        this._addMainAppAction('desktop-icons-settings', () => Prefs.showPreferences());
        this._addMainAppAction('desktop-display-settings', () => this._launchControlCenterPanel('gnome-display-panel.desktop'));
        this._addMainAppAction('file-open', () => this.fileItemMenu.activateNativeMenuAction('open'));
        this._addMainAppAction('file-stack-toggle', () => this.fileItemMenu.activateNativeMenuAction('stack-toggle'));
        this._addMainAppAction('file-open-with', () => this.fileItemMenu.activateNativeMenuAction('open-with'));
        this._addMainAppAction('file-launch-dgpu', () => this.fileItemMenu.activateNativeMenuAction('launch-dgpu'));
        this._addMainAppAction('file-run-program', () => this.fileItemMenu.activateNativeMenuAction('run-program'));
        this._addMainAppAction('file-cut', () => this.fileItemMenu.activateNativeMenuAction('cut'));
        this._addMainAppAction('file-copy', () => this.fileItemMenu.activateNativeMenuAction('copy'));
        this._addMainAppAction('file-rename', () => this.fileItemMenu.activateNativeMenuAction('rename'));
        this._addMainAppAction('file-trash', () => this.fileItemMenu.activateNativeMenuAction('trash'));
        this._addMainAppAction('file-delete-permanently', () => this.fileItemMenu.activateNativeMenuAction('delete-permanently'));
        this._addMainAppAction('file-allow-launching', () => this.fileItemMenu.activateNativeMenuAction('allow-launching'));
        this._addMainAppAction('file-empty-trash', () => this.fileItemMenu.activateNativeMenuAction('empty-trash'));
        this._addMainAppAction('file-eject', () => this.fileItemMenu.activateNativeMenuAction('eject'));
        this._addMainAppAction('file-unmount', () => this.fileItemMenu.activateNativeMenuAction('unmount'));
        this._addMainAppAction('file-extract-here', () => this.fileItemMenu.activateNativeMenuAction('extract-here'));
        this._addMainAppAction('file-extract-to', () => this.fileItemMenu.activateNativeMenuAction('extract-to'));
        this._addMainAppAction('file-send-to', () => this.fileItemMenu.activateNativeMenuAction('send-to'));
        this._addMainAppAction('file-compress', () => this.fileItemMenu.activateNativeMenuAction('compress'));
        this._addMainAppAction('file-new-folder-selection', () => this.fileItemMenu.activateNativeMenuAction('new-folder-selection'));
        this._addMainAppAction('file-properties', () => this.fileItemMenu.activateNativeMenuAction('properties'));
        this._addMainAppAction('file-show-in-files', () => this.fileItemMenu.activateNativeMenuAction('show-in-files'));
        this._addMainAppAction('file-open-terminal', () => this.fileItemMenu.activateNativeMenuAction('open-terminal'));
    }

    _launchControlCenterPanel(desktopFileId) {
        const desktopFile = Gio.DesktopAppInfo.new(desktopFileId);
    ...
    _createGridWindows() {
        ...
        for (let desktopIndex in this._desktopList) {
            ...
            if (this._asDesktop) {
                ...
                desktopName = `Etxe Desktop ${desktop.monitorIndex + 1}`;
            }
    ...
    _showNativeDesktopMenu(x, y) {
        return this.showNativeMenu(x, y, [
            ...
            this._nativeMenuSeparator(),
            this._nativeMenuItem(_('Change Background…'), 'desktop-change-background'),
            this._nativeMenuItem(_('Desktop Icons Settings'), 'desktop-icons-settings'),
            this._nativeMenuItem(_('Display Settings'), 'desktop-display-settings'),
        ]);
    }

    showNativeMenu(x, y, items) {
        try {
            if (!DBusUtils.extensionControl.has_action('showDesktopMenu')) {
                return false;
            }

            DBusUtils.extensionControl.activate_action('showDesktopMenu', new GLib.Variant('a{sv}', {
                x: new GLib.Variant('i', Math.floor(x)),
                y: new GLib.Variant('i', Math.floor(y)),
                items: new GLib.Variant('aa{sv}', items),
            }));
            return true;
        } catch (e) {
            console.error(e, 'Unable to show native Etxe desktop menu.');
            return false;
        }
    }

    _nativeMenuItem(label, action, sensitive = true) {
        return {
            kind: new GLib.Variant('s', 'item'),
            label: new GLib.Variant('s', label),
            action: new GLib.Variant('s', action),
            sensitive: new GLib.Variant('b', sensitive),
        };
    }

    _nativeMenuSeparator() {
        return {
            kind: new GLib.Variant('s', 'separator'),
        };
    }
